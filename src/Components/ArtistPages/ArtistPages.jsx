...
const ArtistPages = () => {
    ...
    const [artistRelated, setArtistRelated] = React.useState([])
    ...
    React.useEffect(() => {
        if (accessToken)
            axios
                .get(
                    `https://api.spotify.com/v1/artists/${id}/related-artists?limit=0`,
                    {
                        headers: {
                            Authorization: 'Bearer ' + accessToken,
                        },
...
console.log(artistRelated)
    return (
        <>
            {loading ? (
                ...
                <div className={style.main}>
                    <div className="main_artist">
                        ...
                        <div className="popular_track">
                            <div className="main__content_trending_artist_title_related">
                                {artistRelated.artists.slice(0,10).map((item, index) => {
                                    return (
                                        <ArtistRelated {...item} index={index} key={index} />
                                    )
                                })}
                            </div>
                        </div>
            ...
            {
                !urlForAudioTrack.length ? (<div></div>) : (
                    <PlayerSearch
